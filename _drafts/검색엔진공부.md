...
- implicit rules: 미리 내포된 규칙으로 정확한 파일명을 쓰지 않고 파일을 다루는 것이 가능
...
11. 정적 라이브러리, 동적 라이브러리
 ...
 - 여러 단계로 링크를 하는 경우
  => 최종 바이너리를 만들기까지 컴파일과 링킹이 여러번 필요할 때, 최초의 컴파일이나 링킹에서 동적 라이브러리를 붙여도 다음이나 최종 링킹 단계에서 해주지 않으면 연결되지 않는다. 중간에 지나치는 링킹 과정마다 항상 동적 라이브러리를 링킹해줘야 링크 문제가 발생하지 않는다는 것을 기억할 것.
...
14. 리눅스(linux) 새로 배운 명령어
 ...
 - gdb: c 디버거. core 파일 해석, 브레이크 포인트, 포인터 변수 참조 메모리 확인 등
  ...
  => core 파일을 넣지 않아도 디버거로 여전히 사용이 가능하다.
 ...
 ii). 아파치 모듈 프로그래밍 관련
  ...
  - 아파치 빌드 및 설치 step by step
   ...
   => 기타 참고 문서
    ...
    - request_rec 레퍼런스: https://ci.apache.org/projects/httpd/trunk/doxygen/structrequest__rec.html
...
35. x-forwarded-for
 ...
 - 참고
  => X-Forwarded-For 개요: http://lesstif.com/pages/viewpage.action?pageId=20775886
...
41. linux shellscript - getopt vs. getopts
 ...
 - getopts: 셸 빌트인. bash 예제가 많지만 sh 에서도 작동하는 듯. 롱 옵션 비지원. 좀 더 새로운 것. 문법이 더 단순함
 => 둘 차이 예제: http://blog.onetechnical.com/2012/07/16/bash-getopt-versus-getopts/
 => getopts 예제: http://wiki.bash-hackers.org/howto/getopts_tutorial

42. jansson
 - json 파싱 라이브러리
 - 순수한 c 라이브러리를 원한다면 좋은 선택

43. JSONObject
 - NGrinder에서 json 파싱할 때 쓰는 라이브러리
 - api 레퍼런스 찾기가 조금 까다로운 편
 - 레퍼런스: http://www.javadoc.io/doc/org.json/json/20160212

44. SetUid와 SetGid
 - 실행순간에만 실 사용자 uid가 프로그램 소유자의 uid로 변경되도록 하는 기법
 - 대표적인 명령어는 /etc/passwd -> root 권한이 있어야 편집 가능하지만 setuid 덕에 일반 사용자가 접근 가능
 - 아파치 httpd 프로그램에 setuid를 주면 일반사용자가 root 권한으로 실행하여 80/443 포트 사용 가능
 - https://www.linux.co.kr/home/lecture/?leccode=476
 - http://eunguru.tistory.com/115
 - 유저/그룹 관리: http://sksstar.tistory.com/8

45. 공유메모리
 - 여러개의 프로세스가 공유할 수 있는 메모리. 멀티프로세스에서 핵심적인 개념
 - http://www.joinc.co.kr/w/Site/system_programing/IPC/SharedMemory

46. 세마포어
 - 뮤텍스와 핵심적인 차이점은, 적용 프로그래밍 기법이 멀티쓰레드인가, 멀티프로세스인가로 나뉜다
...
47. cron job을 시스템 재부팅 직후에 작동시키기
 - cron job은 일정한 시간마다 작업을 스케쥴링할 뿐 재부팅 직후 작동시키는 방법이 없어 보인다
 - 그러나 있다 -> @reboot 키워드
